...
export const PlayPage: React.FC = () => {
  ...
  const [selectedMode, setSelectedMode] = useState<"human" | "bot">("human");
  ...
  const loadGames = async () => {
    try {
      ...
      setGames([]);
      // Keep the page usable for quick start / bot mode even if listing fails.
      setError(null);
    } finally {
      setLoading(false);
    }
  ...
  const handleCreateGame = async () => {
    try {
      ...
      if (response.data?.data?.game_id) {
        ...
        logger.info("PlayPage: Game created successfully", {
          gameId: response.data.data.game_id,
        });
        navigate(`/game/${response.data.data.game_id}`);

        // Show shareable link only for human-vs-human games
        if (!isBotMode) {
          if (navigator.share) {
            try {
  ...
  const multiplayerGames = games.filter((game) => !game.is_vs_bot);
  const waitingGames = multiplayerGames.filter(
    (game) => game.game_state.status === "active" && !game.player_black,
  );
  const activeGames = multiplayerGames.filter(
    (game) => game.game_state.status === "active" && game.player_black,
  );
            ...
            >
                ...
                >
                  <button
                    ...
                    style={{
                      ...
                      transition: "all 0.2s ease",
                      transform:
                        selectedColor === "white" ? "scale(1.05)" : "scale(1)",
                      boxShadow:
                        selectedColor === "white"
              ...
              >
                ...
                >
                  ...
                  <span>
                    {selectedMode === "bot"
                      ? "Bot games are private and cannot be joined by other players."
